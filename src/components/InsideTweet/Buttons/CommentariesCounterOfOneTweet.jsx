...
export default function CommentariesCounterOfOneTweet(props) {
  const { IdTweet, commentaryCount, onChangeEncoreLeCompteur } = props;
  ...
  const getCommentariesOfThisTweet = async () => {
    ...
    setCountCommentaries(
      Object.values(dataCommentaries).filter(
        (commentary) => commentary.commentaryOf === IdTweet
      ).length
    );
